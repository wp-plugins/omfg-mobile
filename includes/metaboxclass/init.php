		...
		foreach ( $this->_meta_box['fields'] as $field ) {
			...
			switch ( $field['type'] ) {
				...
				case 'post_type':
					...
					echo '<h5 class="cmb_metabox_title">',$the_title,' Page Options</h5>';
					echo '<p class="cmb_metabox_description">This site is using the ',$formattedtheme,'</p>';
					echo '<br />';
					break;
				...
				case 'mobile_page_select':
					...
					if (empty($posts)) { 
						...
						echo '<option>No Pages Exist. Go on, create some. . .</option>';
						...
						foreach ($posts as $p) { 
							...
							$thepermalink = get_permalink($p->ID);
				...
				case 'menu_select':
					...
					foreach( (array) $nav_menus as $key => $_nav_menu ) {
						...
						echo '<option value="';
						echo esc_html( $_nav_menu->slug ).'"';
						if (esc_html( $_nav_menu->slug ) == $meta) echo ' selected="selected" ';
						echo '>';
